...
interface CreateGroupGameModalProps {
  groupId: string;
  onGameCreated: (gameId: string) => void;
  onClose: () => void;
}

export function CreateGroupGameModal({ groupId, onGameCreated, onClose }: CreateGroupGameModalProps) {
  ...
  const handleSubmit = async (config: GameConfigInput) => {
    ...
    setError('');
    try {
      const result = await api.createGroupGame(groupId, config);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-lamo-dark">New Group Game</h2>
          <button
            onClick={onClose}
